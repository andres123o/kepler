...
const EncuestaScat: React.FC = () => {
    ...
    const questions = [
        {
            id: 'satisfaction',
            title: '¿Cómo calificarías tu satisfacción general?',
            type: 'stars',
            required: true,
            autoAdvance: true
        },
        {
            id: 'understanding',
            title: '¿Qué tan bien entendió el agente tu problema?',
            subtitle: '1 = No entendió, 5 = Entendió perfectamente',
            type: 'stars',
            required: true,
            autoAdvance: true
        },
        {
            id: 'empathy',
            title: '¿Qué tan amable y empático fue el agente?',
            type: 'stars',
            required: true,
            autoAdvance: true
        },
        {
            id: 'problemResolved',
            title: '¿Tu problema fue resuelto en esta interacción?',
            type: 'yesno',
            required: true,
            autoAdvance: true
        },
        {
            id: 'additional',
            title: '¿Tienes algún comentario o sugerencia para nuestro agente o el equipo de soporte?',
            type: 'text-short',
            required: false,
            autoAdvance: false
        },
    ...
    const handleSubmit = () => {
        ...
        const surveyId = `scat_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        ...
        const existingSurveys = JSON.parse(localStorage.getItem('scatSurveys') || '[]');
        
        // Agregar nueva encuesta
        existingSurveys.push(surveyData);
        
        // Guardar en localStorage
        localStorage.setItem('scatSurveys', JSON.stringify(existingSurveys));
        
        // También guardar la última encuesta para acceso rápido
        localStorage.setItem('lastScatSurvey', JSON.stringify(surveyData));
    ...
    const renderStarRating = (field: 'satisfactionRating' | 'understandingRating' | 'empathyRating') => {
    ...
    const renderYesNoQuestion = () => {
        return (
            <div className="flex justify-center gap-6">
                <motion.button
                    onClick={() => handleResponseChange('problemResolved', 'Sí')}
                    className={`px-8 py-4 rounded-xl font-semibold text-lg transition-all duration-300 ${
                        responses.problemResolved === 'Sí'
                ...
                <motion.button
                    onClick={() => handleResponseChange('problemResolved', 'No')}
                    className={`px-8 py-4 rounded-xl font-semibold text-lg transition-all duration-300 ${
                        responses.problemResolved === 'No'
    ...
    const canProceed = currentQ.required ? 
        (currentQ.type === 'yesno' ? responses.problemResolved !== '' : 
         currentQ.type === 'stars' ? responses[currentQ.id as keyof typeof responses] !== 0 :
         responses[currentQ.id as keyof typeof responses] !== '') :
        true;
                ...
                <div className="flex-1 flex items-center justify-center p-8">
                    ...
                    >
                        ...
                        >
                            ...
                            {currentQ.type === 'stars' && renderStarRating(currentQ.id as 'satisfactionRating' | 'understandingRating' | 'empathyRating')}
                            {currentQ.type === 'yesno' && renderYesNoQuestion()}
